...
<script type="text/javascript">
	...
	dojo.ready(function() {
		...
		**/
		...
		pMenu.addChild(new dijit.MenuItem({
			...
			onClick : function(item) {
				var xhrArgs = {
					  	url: "<c:url value='/softwarefolder/'/>" + tree.selectedNode.item.id,
					  	handleAs: "json",
					  	content:{
					  		_method : "delete"
					  	},
					  	load: function(data){
					  		if(data.success){
					  			...
					  			restModel.get(selected.getParent().item.id).then(function(fullNode){
	...
	function rowOperationFormatter(id,index){
		...
		'<button data-dojo-type="dijit.form.Button" onclick="deleteFile(' + id + ')">删除</button>';
	}
...
<div id="file_main" data-dojo-type="dijit.layout.BorderContainer" style="width:100%;height:100%;margin:0;overflow: hidden;">
	...
	<div id="file_left" data-dojo-type="dijit.layout.ContentPane" 
		...
		<div id="upDlg" data-dojo-type="dijit.Dialog" title="上传文件">
			...
			<form id="upload_form" action="<c:url value="/software/upload"/>"
				enctype="multipart/form-data" method="POST">
				<!-- 父文件夹id -->
				<input type="hidden" id="uploader_parentId" name="parentId">
				<!-- 上传文件选择器 -->
				<input name="file" multiple="false" type="file" force="iframe"
					data-dojo-type="dojox.form.Uploader" label="选择文件" id="uploader" data-dojo-props="iconClass:'dijitCommonIcon dijitIconFolderOpen'">
				...
				<!-- 上传文件列表 -->
				<div data-dojo-type="dojox.form.uploader.FileList"	uploaderId="uploader"></div>
			</form>
		</div>
	</div>
