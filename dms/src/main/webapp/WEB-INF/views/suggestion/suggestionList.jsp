...
dojo.ready(function(){
	var store = new dojo.store.JsonRest({
		target : "<c:url value='/suggestion/suggestionList'/>",
		sortParam : "sort"
	});
    ...
    var layout = [[
      ...
      {name: '标题', field: 'suggestionTitle', width:"40%"},
      {name: '作者', width:"15%"},
      {name: '发布时间', field: 'suggestionDate', width:"15%", formatter:dateFormatter},
...
function optionFormatter(id){
	return "<a href='<%=request.getContextPath() %>/suggestion/" + id + "'>阅读</a>&nbsp;&nbsp;"
			+"<a href='#' onclick='delSuggestion("+id+")'>删除</a>";
}

function add(){
	window.location.href="<c:url value='/suggestion/create'/>";
}
